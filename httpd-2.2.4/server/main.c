...
static void destroy(process_rec* process)
{
...
EXPORT_C int apache_main(int argc, const char * const argv[])
#else
...
#else
    /*
     * On Symbian we must allow the control to return. Otherwise
     * the termination of the Symbian server, in whose context this 
     * code is run, is quite ugly.
     */
    destroy(process);
#endif
