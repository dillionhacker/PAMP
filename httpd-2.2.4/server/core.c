...
/* There seems to be some problem with non-blocking sockets on Open-C.
 * Consquently there's a boolean directive "AlwaysUseBlockingIO", using 
 * which we can force blocking IO on. In fact, currently the default is 
 * to _use_ blocking IO and with the directive that approach can be 
 * turned off.
 */
int ap_always_use_blocking_io = 1;
#endif
...
AP_DEFINE_EXPORTED_DATA(ap_filter_rec_t *, ap_subreq_core_filter_handle);
AP_DEFINE_EXPORTED_DATA(ap_filter_rec_t *, ap_core_output_filter_handle);
AP_DEFINE_EXPORTED_DATA(ap_filter_rec_t *, ap_content_length_filter_handle);
AP_DEFINE_EXPORTED_DATA(ap_filter_rec_t *, ap_core_input_filter_handle);
...
#if defined(__SYMBIAN32__)
static const char *ap_set_always_use_blocking_io(cmd_parms *cmd, void *mconfig,
                                                 int val) {
    if (val)
        ap_always_use_blocking_io = 1;
    else
        ap_always_use_blocking_io = 0;
...
#if defined(__SYMBIAN32__)
AP_INIT_FLAG(
        "AlwaysUseBlockingIO", ap_set_always_use_blocking_io, NULL, RSRC_CONF,
        "Whether or not blocking IO should always be used. Default is On."),
#endif
{ NULL }
};
...
AP_DEFINE_EXPORTED_DATA(module, core_module) = {
