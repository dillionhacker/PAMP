...
AP_DEFINE_EXPORTED_DATA(const char *, ap_server_argv0) = NULL;

AP_DEFINE_EXPORTED_DATA(const char *, ap_server_root) = NULL;

AP_DEFINE_EXPORTED_DATA(apr_array_header_t *, ap_server_pre_read_config) = NULL;
AP_DEFINE_EXPORTED_DATA(apr_array_header_t *, ap_server_post_read_config) = NULL;
AP_DEFINE_EXPORTED_DATA(apr_array_header_t *, ap_server_config_defines) = NULL;

AP_DEFINE_EXPORTED_DATA(ap_directive_t *, ap_conftree) = NULL;
...
AP_DEFINE_EXPORTED_DATA(module *, ap_top_module) = NULL;
AP_DEFINE_EXPORTED_DATA(module **, ap_loaded_modules)=NULL;
...
AP_DECLARE(module *) ap_find_linked_module(const char *name)
{
    ...
    for (modp = ap_top_module; modp; modp = modp->next) {
        ...
        /* When using armv5, __FILE__ correctly expands into the filename, 
         * but apparently _always_ with an initial capital letter, regardless
         * of whether the filename has an initial capital letter or not.
         */
        if (strcasecmp(modp->name, name) == 0)
#else
