...
AP_DECLARE_EXPORTED_DATA(const char *, ap_server_argv0);
/** The global server's ServerRoot */
AP_DECLARE_EXPORTED_DATA(const char *, ap_server_root);

/* for -C, -c and -D switches */
/** An array of all -C directives.  These are processed before the server's
 *  config file */
AP_DECLARE_EXPORTED_DATA(apr_array_header_t *, ap_server_pre_read_config);
/** An array of all -c directives.  These are processed after the server's
 *  config file */
AP_DECLARE_EXPORTED_DATA(apr_array_header_t *, ap_server_post_read_config);
/** An array of all -D defines on the command line.  This allows people to
 *  effect the server based on command line options */
AP_DECLARE_EXPORTED_DATA(apr_array_header_t *, ap_server_config_defines);
