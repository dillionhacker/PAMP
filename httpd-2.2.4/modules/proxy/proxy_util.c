...
static int proxy_match_ipaddr(struct dirconn_entry *This, request_rec *r)
{
    ...
    struct apr_sockaddr_t *reqaddr;
        ...
        if (symbian_iap_sockaddr_info_get(&reqaddr,
                                          conf->iap,
                                          host, APR_UNSPEC, 0, 0, r->pool)
#else
...
/* Return TRUE if host represents a host name */
PROXY_DECLARE(int) 
ap_proxy_is_hostname_iap(const char *iap, 
                         struct dirconn_entry *This, apr_pool_t *p)
{
    ...
    if (host[i] != '\0' || 
        symbian_iap_sockaddr_info_get(&addr, iap, host,
                                      APR_UNSPEC, 0, 0, p) != APR_SUCCESS)
        return 0;
...
#if defined(__SYMBIAN32__)
        symbian_socket_iap_assign(*newsock, conf->iap);
#endif
...
#if defined(__SYMBIAN32__)
        err = symbian_iap_sockaddr_info_get(&(conn->addr),
                                            conf->iap,
...
#if defined(__SYMBIAN32__)
        symbian_socket_iap_assign(newsock, 
                                  ((proxy_server_conf*) 
