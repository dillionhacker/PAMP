...
static void * merge_proxy_config(apr_pool_t *p, void *basev, void *overridesv)
{
    ...
    ps->pool = p;
#if defined(__SYMBIAN32__)
    ps->iap = (overrides->iap == NULL) ? base->iap : overrides->iap;
#endif
    return ps;
}
...
#if defined(__SYMBIAN32__)
        else if (ap_proxy_is_hostname_iap(conf->iap, New, parms->pool)) {
#else
...
static void check_proxy_iap(const char* iap)
{
    ...
    if (sd != -1) {
        ...
        if (rv == 0) {
            ...
            if (rv == 0) {
                ...
                for (i = 0; i < n; i++) {
                    struct ifreq
                        *ifr = (struct ifreq*) (ifc.ifc_buf + i * sizeof(*ifr));
                    
                    if (strcmp(ifr->ifr_name, iap) == 0)
                        break;
                }
                
                if (i == n)
                    ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL,
                                 "ProxyIAP: Could not find specified "
                                 "IAP \"%s\".", iap);
            }
        ...
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL,
                     "ProxyIAP: Could not create socket for finding "
                     "out the available access points.");
}


static 
const char* set_proxy_iap(cmd_parms *parms, void *dummy, const char *arg)
{
    proxy_server_conf *psf =
    ap_get_module_config(parms->server->module_config, &proxy_module);
    ...
    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, NULL,
                 "ProxyIAP: Using \"%s\" as proxy IAP.", arg);
...
#if defined(__SYMBIAN32__)
    AP_INIT_TAKE1("ProxyIAP", set_proxy_iap, NULL, RSRC_CONF,
                  "the internet access point to be used when proxying."),
#endif
...
AP_EXPORT_1_SYMBOL(proxy_module)

#if defined(__SYMBIAN32__)
PROXY_DECLARE(module*) _proxy_module(void)
{
  return &proxy_module;
}
#endif
