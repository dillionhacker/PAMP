...
static dav_error * dav_fs_walker(dav_fs_walker_context *fsctx, int depth)
{
    ...
    while ((apr_dir_read(&dirent, APR_FINFO_DIRENT, dirp)) == APR_SUCCESS) {
        ...
        /* FIXME: Symbian's platform security causes the stating of
         * FIXME: certain directories to fail with EACCESS, if the 
         * FIXME: capabilities of the application are not sufficient.
         * FIXME: Hence we need to gracefully recover from that.
         */
        if (status == EACCES) {
            continue;
        }
