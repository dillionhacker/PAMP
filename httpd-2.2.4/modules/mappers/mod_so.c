...
{
    const char
        *file = path + strlen(path); // file points to the NULL

    if (file != path)
        --file; // Points to the last character.
...
typedef apr_status_t (*ap_lookup_function_t)(const char* pName,
                                             void**      ppSymbol);

apr_status_t symbian_dso_sym(apr_dso_handle_sym_t* ressym, 
                             ...
                             const char*           symname)
{
    ...
    /*
     * Symbian does not yet allow the loading of symbols by name. 
     * Instead we must first load (by convention) the function at
     * ordinal 1, and use it for actually looking up the symbol.
     */

    apr_status_t
        rc = apr_dso_sym(&symbol, handle, "1");

    if (rc == APR_SUCCESS) {
        ap_lookup_function_t
            lookup = (ap_lookup_function_t) symbol;
        
        rc = lookup(symname, ressym);
    }
    
    return rc;
}

#define apr_dso_sym symbian_dso_sym
                               ...
                               const char *modname, const char *filename)
{
    ...
    module *modp;
#if defined(__SYMBIAN32__)
    /* By default, modules are loaded from a sub-directory "modules" under
     * the Apache installation directory. On Symbian, binaries can _only_
     * be loaded from /Sys/Bin, but so as to not require modifications in
     * httpd.conf we'll just silently remove any directory components
     * in the path.
     */
    const char *szModuleFile = basename(filename);
#else
