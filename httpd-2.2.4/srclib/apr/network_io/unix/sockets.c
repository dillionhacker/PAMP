...
#if defined(__SYMBIAN32__)
    if (thesocket->iap_started) {
        struct ifreq ifr;

        strcpy(ifr.ifr_name, thesocket->iap);

        /* No need to worry about return value. Can't do anything about it
         * anyway.
         */
        ioctl(thesocket->socketdes, SIOCIFSTOP, &ifr);
    }
    
    thesocket->iap = 0;
    thesocket->iap_started = 0;
#endif
...
#if defined(__SYMBIAN32__)
    if (apr_is_option_set(sock, SYMBIAN_SO_ALWAYS_BLOCK) == 1) {
      apr_set_option(*new, SYMBIAN_SO_ALWAYS_BLOCK, 1);
    }
#endif
...
#if defined(__SYMBIAN32__)
            /* Returns 0, but does not set 'error'.
             */
            error = 0;
#endif
