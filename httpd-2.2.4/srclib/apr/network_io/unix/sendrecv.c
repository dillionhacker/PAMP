                             ...
                             apr_size_t *len)
{
    ...
    apr_socket_opt_get(sock, SYMBIAN_SO_ALWAYS_BLOCK, &block);
    apr_socket_opt_get(sock, APR_SO_NONBLOCK, &on);
...
#else
        /* FIXME: Inside apr_wait_for_io_or_timeout(), there's a 
         * FIXME: timeout call to select that for, whatever reason, 
         * FIXME: returns _immediately_ with 0 (i.e. timed out) and
         * FIXME: no socket ready.
         * FIXME: 
         * FIXME: If that is ignored and we continue, then quite
         * FIXME: soon there will be a E32User-CBase 46 panic.
         * FIXME: So, for the time being we just pretend it always 
         * FIXME: succeeds, as it seems to work.
         */
        arv = APR_SUCCESS;
#endif
        if (arv != APR_SUCCESS) {
