...
extern int gethostbyname_iap_r(const char*      iap,
                               const char*      name,
                               struct hostent*  ret,
                               char*            buf,
                               size_t           buflen,
...
#define GETHOSTBYNAME_R_GLIBC2
#undef  GETHOSTBYNAME_IS_THREAD_SAFE
#define HAVE_GETHOSTBYNAME_R

#if defined(HAVE_GETADDRINFO)
#error You must add support for the "domainname:interface" syntax, before you can use getaddrinfo.
#endif
...
#if defined(__SYMBIAN32__)
        gethostbyname_iap_r(iap, hostname, &hs, tmp, GETHOSTBYNAME_BUFLEN - 1, 
                            &hp, &hosterror);
#else
        /* Linux glibc2+ */
                                                ...
                                                apr_int32_t flags, apr_pool_t *p)
{
    ...
    *sa = NULL;
    ...
    return find_addresses(sa, 
#if defined(__SYMBIAN32__)
                          0,
#endif
                          hostname, family, port, flags, p);
}
                                                        ...
                                                        apr_pool_t     *p)
{
    ...
    *sa = NULL;
    ...
    return find_addresses(sa, iap, hostname, family, port, flags, p);
}
