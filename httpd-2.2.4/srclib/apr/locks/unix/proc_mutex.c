...
#else
    if (sem_destroy(mutex->psem_interproc) < 0) {
        return errno;
    }
#endif
                                            ...
                                            const char *fname)
{
    ...
    sem_unlink(semname);
#else /* __SYMBIAN32__ */
    psem = apr_pcalloc(new_mutex->pool, sizeof(*psem));

    if (sem_init(psem, 0, 1) == -1)
        return errno;
#endif
                                                   ...
                                                   const char *fname)
{
    ...
    close(fd);
#else /* __SYMBIAN32__ */
    new_mutex->pthread_interproc = 
        apr_pcalloc(new_mutex->pool, 
                    sizeof(*new_mutex->pthread_interproc));
#endif
