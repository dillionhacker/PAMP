...
#define SYMBIAN_SO_ALWAYS_BLOCK 65536 /**< Always use blocking IO, regardless of
                                       * the value of APR_SO_NONBLOCK.
                                       */
#endif
/** @} */
...
#if defined(__SYMBIAN32__)
/**
 * Assign the specified IAP to the socket, and start the IAP.
 * @param sock The socket whose IAP should be set.
 * @param iap  The name of the IAP or NULL.
 * @return APR_SUCCESS if the IAP is successfully associated with the
 * socket and started, or if no IAP has been assigned and iap is NULL.
 * @return APR_EINVAL if a non-NULL iap has already been assigned.
 */
...
#if defined(__SYMBIAN32__)
APR_DECLARE(apr_status_t) symbian_iap_sockaddr_info_get(apr_sockaddr_t **sa,
                                                        const char *iap,
                                                        const char *hostname,
                                                        apr_int32_t family,
                                                        apr_port_t port,
 ...
 *             the accept filter. 
 */
APR_DECLARE(apr_status_t) apr_socket_accept_filter(apr_socket_t *sock, 
                                                   char *name,
                                                   char *args);
#endif
