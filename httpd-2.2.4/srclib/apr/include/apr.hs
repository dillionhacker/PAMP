 ...
 *
 * You must modify apr.hs instead.
 *
 * And please, make an effort to stub apr.hw, apr.hnw and apr.h.in in the 
 * process.
 *
 * This is the Symbian specific version of apr.h.  It is copied from
 * apr.hs by bldmake due to instructions in ../build/bld.inf.
 */
 ...
 * ./configure, or copied from include/apr.hw, include/apr.hnw or
 * include/apr.hs for Win32, Netware or Symbian by those build environments, 
 * respectively.
 */
...
#error Remove this ifdef completely.
#else
/*
 * Needed in apr_pool.c when cleaning up processes as part of
 * pool cleanup. That won't ever happen on Symbian, but these
 * defines are needed nontheless.
 */
...
// FIXME: What is corkable TCP? JWi
...
// FIXME: It's unclear for me to what extent IPv6 is supported. JWi
...
// Structure exists, but functions rlimit functions do not.
...
// FIXME: For the time being. JWi
#define APR_HAS_THREADS           1
#define APR_HAS_SENDFILE          0
#define APR_HAS_MMAP              0
#define APR_HAS_FORK              0
#define APR_HAS_RANDOM            1
// FIXME: What is this? JWi
...
// FIXME: What is the unsigned version of this?! JWi
#define APR_UINT64_C(val) (val##i64)
#elif defined(__GCCE__) || defined(__GCC32__)
#define APR_INT64_C(val)  (val##LL)
#define APR_UINT64_C(val) (val##LLU)
#elif defined(__ARMCC__)
#define APR_INT64_C(val)  (val##ll)
#define APR_UINT64_C(val) (val##llu)
#else
#error Dont know how to express 64-bit constants on this platform.
#endif
...
#define APR_DECLARE(type)            EXPORT_C type
#define APR_DECLARE_NONSTD(type)     EXPORT_C type
#define APR_DECLARE_DATA
#else
#define APR_DECLARE(type)            IMPORT_C type
#define APR_DECLARE_NONSTD(type)     IMPORT_C type
#define APR_DECLARE_DATA
#endif
...
// FIXME: What is actually stated here? JWi
...
/**
 * Logs data using Symbian's RFileLogger.
 *
 * @param dir  Subdirectory of C:\Logs.
 * @param file File in C:\Logs\<dir>.
 *
 * @remark If <dir> does not exist, then the logged data is simply
 * discarded.
 * 
 */
APR_DECLARE(void) symbian_log_append(const char* dir,
                                     const char* file,
                                     const char* format, ...);


#if defined(APR_DECLARE_EXPORT)
#define APR_DECLARE_EXPORTED_DATA_ARRAY(type, name, dim)\
APR_DECLARE_DATA extern type name dim

#define APR_DEFINE_EXPORTED_DATA_ARRAY_FUNC(type, name, dim)\
APR_DECLARE(type) (* _ ## name(void)) dim\
{\
    return &name;\
}

#if !defined(__CW32__)
#define APR_DEFINE_EXPORTED_CONST_DATA_ARRAY_FUNC(type, name, dim)\
APR_DEFINE_EXPORTED_DATA_ARRAY_FUNC(const type, name, dim)
#else
#define APR_DEFINE_EXPORTED_CONST_DATA_ARRAY_FUNC(non_const_type, name, dim)\
APR_DECLARE(const non_const_type) (* _ ## name(void)) dim\
{\
    return (non_const_type (*) dim ) &name;\
}
#endif

#define APR_DEFINE_EXPORTED_DATA_ARRAY(type, name, dim)\
APR_DEFINE_EXPORTED_DATA_ARRAY_FUNC(type, name, dim)\
APR_DECLARE_DATA type name dim

#define APR_DEFINE_EXPORTED_CONST_DATA_ARRAY(non_const_type, name, dim)\
APR_DEFINE_EXPORTED_CONST_DATA_ARRAY_FUNC(non_const_type, name, dim)\
APR_DECLARE_DATA const non_const_type name dim
 ...
 * apr_time.h
 */
#define apr_month_snames (*_apr_month_snames())
#define apr_day_snames   (*_apr_day_snames())
