...
#if !defined(HAVE_TERMIOS_H) && !defined(WIN32) && !defined(NETWARE) && !defined(__SYMBIAN32__)
/*
...
/*
 * SYMBIAN: Symbian does have a console, but it would be better to use
 * SYMBIAN: the GUI functionality that is always present. For the time
 * SYMBIAN: it's not supported at all.
 */
static char *getpass(const char *prompt)
{
    return NULL;
