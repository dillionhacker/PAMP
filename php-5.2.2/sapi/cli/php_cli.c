...
#endif /* HAVE_LIBREADLINE || HAVE_LIBEDIT */
#if 1 // Symbian testing
			{
				FILE* newStdout = NULL;
				char* outputFileName = "c:\\Data\\PHP\\php_output.txt";
		        newStdout = freopen(outputFileName, "w", stdout);
				if( newStdout == NULL ) {
					printf("OOPS, freopen returned NULL\n\n");
				}
			}
#endif // Symbian testing
