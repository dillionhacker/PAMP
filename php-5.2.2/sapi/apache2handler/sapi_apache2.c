...
php_apache_sapi_read_post(char *buf, uint count_bytes TSRMLS_DC)
{
	...
	strncpy(buffer, buf - tlen, sizeof(buffer));
	buffer[tlen] = 0;
...
// FIXME: This version is not thread-safe.
#ifndef ZTS
	int threaded_mpm;
...
php_apache_server_startup(apr_pool_t *pconf, apr_pool_t *plog, apr_pool_t *ptemp, server_rec *s)
{
	...
	if (data == NULL) {
		...
		// S60 Apache crashes if apr_pool_cleanup_null is called. 
		apr_pool_userdata_set((const void *)1, userdata_key, 0 /*apr_pool_cleanup_null*/, s->process->pool);
