...
PHP_FUNCTION(uniqid)
{
...
#if HAVE_USLEEP && !defined(PHP_WIN32) && !defined(__SYMBIAN32__)
	if (!more_entropy) {
#if defined(__CYGWIN__)
	...
	sec = (int) tv.tv_sec;
#if defined(__SYMBIAN32__)
	/* gettimeofday seems to always return 0 as tv_usec on Symbian */
	++count_fake_usec;
	count_fake_usec %= 0x100000;
	usec = count_fake_usec;
#else
