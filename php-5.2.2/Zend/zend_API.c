		...
		switch (c) {
			...
			default:
				if (!quiet) {
					...
					char *class_name = zend_const_cast(char*)(active_function->common.scope ? active_function->common.scope->name : "");
...
ZEND_API int _object_and_properties_init(zval *arg, zend_class_entry *class_type, HashTable *properties ZEND_FILE_LINE_DC TSRMLS_DC)
{
	...
	if (class_type->ce_flags & (ZEND_ACC_INTERFACE|ZEND_ACC_IMPLICIT_ABSTRACT_CLASS|ZEND_ACC_EXPLICIT_ABSTRACT_CLASS)) {
		char *what = zend_const_cast(char*)(class_type->ce_flags & ZEND_ACC_INTERFACE ? "interface" : "abstract class");
...
ZEND_API int zend_register_functions(zend_class_entry *scope, zend_function_entry *functions, HashTable *function_table, int type TSRMLS_DC)
{
	...
	while (ptr->fname) {
		...
		lowercase_name = (char*)do_alloca(fname_len+1);
...
ZEND_API int zend_declare_property_null(zend_class_entry *ce, char *name, int name_length, int access_type TSRMLS_DC)
{
	...
	if (ce->type & ZEND_INTERNAL_CLASS) {
		property = (zval*)malloc(sizeof(zval));
	} else {
		ALLOC_ZVAL(property);
	}
	INIT_ZVAL(*property);
...
ZEND_API int zend_declare_class_constant_null(zend_class_entry *ce, char *name, size_t name_length TSRMLS_DC)
{
	...
	if (ce->type & ZEND_INTERNAL_CLASS) {
		constant = (zval*)malloc(sizeof(zval));
	} else {
		ALLOC_ZVAL(constant);
	}
	ZVAL_NULL(constant);
