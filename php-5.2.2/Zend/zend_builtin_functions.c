...
ZEND_FUNCTION(get_class_methods)
{
	...
	if (ZEND_NUM_ARGS() != 1 || zend_get_parameters_ex(1, &method_class)==FAILURE) {
		ZEND_WRONG_PARAM_COUNT();
	}

	if (Z_TYPE_PP(method_class) == IS_OBJECT) {
		/* TBI!! new object handlers */
		if (!HAS_CLASS_ENTRY(**method_class)) {
			RETURN_FALSE;
		}
		ce = Z_OBJCE_PP(method_class);
	} else if (Z_TYPE_PP(method_class) == IS_STRING) {
		if (zend_lookup_class(Z_STRVAL_PP(method_class), Z_STRLEN_PP(method_class), &pce TSRMLS_CC) == SUCCESS) {
			ce = *pce;
		}
	}
...
ZEND_FUNCTION(class_exists)
{
	...
	if (!autoload) {
		lc_name = (char*)do_alloca(class_name_len + 1);
...
ZEND_FUNCTION(interface_exists)
{
	...
	if (!autoload) {
		lc_name = (char*)do_alloca(iface_name_len + 1);
