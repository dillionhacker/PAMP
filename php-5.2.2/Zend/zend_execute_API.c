...
#if defined(ZEND_WIN32) || defined(__SYMBIAN32__)
	EG(timed_out) = 0;
#endif
...
ZEND_API char *get_active_class_name(char **space TSRMLS_DC)
{
	...
	switch (EG(function_state_ptr)->function->type) {
		...
		case ZEND_INTERNAL_FUNCTION:
		{
			...
			if (space) {
				*space = zend_const_cast(char*)(ce ? "::" : "");
			}
			return zend_const_cast(char*)(ce ? ce->name : "");
		}
		default:
			if (space) {
				*space = "";
...
ZEND_API int zend_lookup_class_ex(char *name, int name_length, int use_autoload, zend_class_entry ***ce TSRMLS_DC)
{
	...
	lc_name = (char*)do_alloca(name_length + 1);
...
ZEND_API int zend_eval_string(char *str, zval *retval_ptr, char *string_name TSRMLS_DC)
{
	...
	if (retval_ptr) {
		...
		Z_STRVAL(pv) = (char*)emalloc(Z_STRLEN(pv) + 1);
...
void zend_set_timeout(long seconds)
{
	...
	EG(timed_out) = 0;
	zend_set_timeout_symbian(seconds);
#else
