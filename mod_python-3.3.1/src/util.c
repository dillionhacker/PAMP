...
static module *find_module(char *name)
{
    ...
    for (n = 0; ap_loaded_modules[n]; ++n) {
         ...
         * of whether the filename has a initial capital letter or not.
         *
         * Incidentally, why not use ap_find_linked_module?
         */
        if (strcasecmp(name, ap_loaded_modules[n]->name) == 0)
#else
