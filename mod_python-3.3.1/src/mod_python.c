...
static int python_concurrency = 1;

static apr_thread_mutex_t* concurrency_lock = 0;

#define LOCK_CONDITIONALLY()\
if (!python_concurrency)\
    apr_thread_mutex_lock(concurrency_lock)\

#define UNLOCK_CONDITIONALLY()\
if (!python_concurrency)\
    apr_thread_mutex_unlock(concurrency_lock)\

#define SERIALIZE_CONDITIONALLY(X)\
LOCK_CONDITIONALLY();\
\
do { int rc = X; UNLOCK_CONDITIONALLY(); return rc; } while(0)
...
#if defined(__SYMBIAN32__)
    /* FIXME: PSP should actually be a _regular_ Python extension,
     * FIXME: included via a normal import statement, but for the
     * FIXME: time being we include it in mod_python itself.
     */
    init_psp();
#endif
                       ...
                       apr_pool_t *plog, server_rec *s)
{
    ...
    if (!data) {
        apr_pool_userdata_set((const void *)1, userdata_key,
                              0 /*apr_pool_cleanup_null*/, s->process->pool);
        return OK;
    }

#if defined(__SYMBIAN32__)
    if (PyS60_Initialize(p) != APR_SUCCESS)
...
#endif
#if defined(__SYMBIAN32__)
        /* A nested mutex is often a sign of sloppyness. However, I'm
         * not entirelly sure of the call hierarchy, so I don't dare
         * not use unnested one.
         */
        apr_thread_mutex_create(&concurrency_lock, 
...
#else
    SERIALIZE_CONDITIONALLY(python_filter(1, f, bb, mode, block, readbytes));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_filter(0, f, bb, 0, 0, 0));
#endif
}
...
#if defined(__SYMBIAN32__)
static const char *directive_PythonConcurrency(cmd_parms *cmd, void *mconfig,
                                               int val) {
    if (val)
 ...
 */
#if defined(__SYMBIAN32__)
/* FIXME: The Symbian MPM is multi-threaded but single process, so 
 * FIXME: actually(?) there is no need to run child_inits. However,
 * FIXME: as mod_proxy depends upon its child_init to be run for 
 * FIXME: setup, we must call them. However, when that's done
 * FIXME: save_interpreter below causes an abort in PyThreadState_Get()
 * FIXME: since - for whatever reason - there is no current state at 
 * FIXME: the moment. For the time being we just provide a child_init
 * FIXME: that does nothing.
 *
 */
...
#else
    SERIALIZE_CONDITIONALLY(python_connection(con));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonAccessHandler"));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonAuthenHandler"));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonAuthzHandler"));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonFixupHandler"));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonHandler"));
#endif
}
...
#else
    rc = python_handler(req, "PythonHeaderParserHandler");
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonLogHandler"));
#endif
}
...
#else
    rc = python_handler(req, "PythonPostReadRequestHandler");
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonTransHandler"));
#endif
}
...
#else
    SERIALIZE_CONDITIONALLY(python_handler(req, "PythonTypeHandler"));
#endif
}
...
#if defined(__SYMBIAN32__)
    AP_INIT_FLAG(
        "PythonConcurrency", directive_PythonConcurrency, NULL, RSRC_CONF,
        "Whether or not python scripts should be allowed to run "
        "concurrently in different threads. Default On."),
#endif
    {NULL}
};
...
AP_EXPORT_1_SYMBOL(python_module)
